#packages 'pangaear', 'seacarb' and 'mgcv' need ot be installed
require(pangaear)
require(seacarb)
require(mgcv)
...
#co2_dat<- read.csv("marcott2014.csv")
#co2_dat$co2<- co2_dat$pco2

#calculate pH assuming an alkalinity of anywhere between 2325 and 2475 umol/kg
alk <- 0.002400
c<-carb(flag=24,var1=co2_dat$co2,var2=alk,T=28,S=34.5,P=2.5,Pt=0.000002,Sit=0.00003,k1k2="m06",kf="dg",ks="d",pHscale="SWS",b="u74")
co2_dat$pH <- c$pH


#we now fit the pH data versus age using a GAM in 'mgcv' package which we will use to predict the pH at different time intervals later 
...
#convert to DMg/Ca using -8% per 0.1 pH unit sensitivity
DpH<-p_pH-p_pH[1]
DT_pH <- -0.83*DpH/0.06
...
#salinity
#sealevel correction - lam beck et al 2014
#esl_dat<-read.csv("/Users/williamgray/R/globaldata/lambeck2014_esl.csv")
#sealevel correction - sprat et al 2014
...
#sealevel salinity from adkins, Sl d18o sw - Schrag 2002
...
#dat<-read.csv("data/mohtadi2014_39kl.csv")
#dat$age<-dat$age_ka
...
#mgca data from https://doi.org/10.1594/PANGAEA.833322
...
#predict pH for each time interval using GAM
dat$pH <- predict(m_pH, newdata= dat)

#predict sea level from gam and convert to salinity using modern pH
